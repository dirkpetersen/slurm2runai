# runai training jax bash

open a bash shell in a jax training workload

```
runai training jax bash [WORKLOAD_NAME] [flags]
```
...
runai training jax bash <workload-name>

# Open a bash shell in a specific worker of the workload
runai training jax bash <workload-name> --pod <pod-name>
```
...
```
GET https://run-ai-docs.nvidia.com/self-hosted/reference/cli/runai/runai_training_jax_bash.md?ask=<question>
```
