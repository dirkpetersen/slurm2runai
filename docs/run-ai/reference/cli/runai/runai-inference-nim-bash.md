# runai inference nim bash

open a bash shell in a nim inference workload
...
```
runai inference nim bash [WORKLOAD_NAME] [flags]
```
...
runai inference nim bash <workload-name>

# Open a bash shell in a specific pod of the workload
runai inference nim bash <workload-name> --pod <pod-name>
```
...
```
GET https://run-ai-docs.nvidia.com/self-hosted/reference/cli/runai/runai-inference-nim-bash.md?ask=<question>
```
