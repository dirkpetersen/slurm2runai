# runai inference distributed port-forward

forward one or more local ports to a distributed inference workload

```
runai inference distributed port-forward [WORKLOAD_NAME] [flags]
```
...
runai inference distributed port-forward <workload-name> --port 8080:8090 --address localhost

# Forward connections from 0.0.0.0:8080 to a workload on port 8080:
runai inference distributed port-forward <workload-name> --port 8080 --address 0.0.0.0 [requires privileges]
...
runai inference distributed port-forward <workload-name> --port 8080:8090 --port 6443:443 --address localhost
```
...
```
GET https://run-ai-docs.nvidia.com/self-hosted/reference/cli/runai/runai-inference-distributed-port-forward.md?ask=<question>
```
