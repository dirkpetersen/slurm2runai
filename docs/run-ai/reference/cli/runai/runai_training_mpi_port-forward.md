...
forward one or more local ports to a mpi training workload
...
runai training mpi port-forward <workload-name> --port 8080:8090 --address localhost

# Forward connections from 0.0.0.0:8080 to a workload on port 8080:
runai training mpi port-forward <workload-name> --port 8080 --address 0.0.0.0 [requires privileges]
...
runai training mpi port-forward <workload-name> --port 8080:8090 --port 6443:443 --address localhost
```
...
```
GET https://run-ai-docs.nvidia.com/self-hosted/reference/cli/runai/runai_training_mpi_port-forward.md?ask=<question>
```
