# runai training tensorflow logs

view logs of a tf training workload
...
runai training tf logs <workload-name>

# Get logs for a specific pod in a workload
runai training tf logs <workload-name> --pod=<pod-name>

# Get logs for a specific container in a workload
runai training tf logs <workload-name> --container=<container-name>

# Get the last 100 lines of logs
runai training tf logs <workload-name> --tail=100

# Get logs with timestamps
runai training tf logs <workload-name> --timestamps

# Follow the logs
runai training tf logs <workload-name> --follow

# Get logs for the previous instance of the workload
runai training tf logs <workload-name> --previous

# Limit the logs to 1024 bytes
runai training tf logs <workload-name> --limit-bytes=1024

# Get logs since the last 5 minutes
runai training tf logs <workload-name> --since=300s

# Get logs since a specific timestamp
runai training tf logs <workload-name> --since-time=2023-05-30T10:00:00Z

# Wait up to 30 seconds for a workload to be ready for logs
runai training tf logs <workload-name> --wait-timeout=30s
```
...
```
GET https://run-ai-docs.nvidia.com/self-hosted/reference/cli/runai/runai_training_tensorflow_logs.md?ask=<question>
```
