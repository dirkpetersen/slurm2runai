# runai inference standard port-forward

forward one or more local ports to an inference workload

```
runai inference standard port-forward [WORKLOAD_NAME] [flags]
```
...
```
GET https://run-ai-docs.nvidia.com/self-hosted/reference/cli/runai/runai-inference-standard-port-forward.md?ask=<question>
```
