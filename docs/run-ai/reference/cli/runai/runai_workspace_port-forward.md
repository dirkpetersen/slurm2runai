...
forward one or more local ports to a workspace workload
...
runai workspace port-forward <workload-name> --port 8080:8090 --address localhost

# Forward connections from 0.0.0.0:8080 to a workload on port 8080:
runai workspace port-forward <workload-name> --port 8080 --address 0.0.0.0 [requires privileges]
...
```
GET https://run-ai-docs.nvidia.com/self-hosted/reference/cli/runai/runai_workspace_port-forward.md?ask=<question>
```
