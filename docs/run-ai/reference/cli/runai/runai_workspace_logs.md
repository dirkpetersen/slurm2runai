...
view logs of a workspace workload
...
runai workspace logs <workload-name>

# Get logs for a specific pod in a workload
runai workspace logs <workload-name> --pod=<pod-name>

# Get logs for a specific container in a workload
runai workspace logs <workload-name> --container=<container-name>

# Get the last 100 lines of logs
runai workspace logs <workload-name> --tail=100

# Get logs with timestamps
runai workspace logs <workload-name> --timestamps

# Follow the logs
runai workspace logs <workload-name> --follow

# Get logs for the previous instance of the workload
runai workspace logs <workload-name> --previous

# Limit the logs to 1024 bytes
runai workspace logs <workload-name> --limit-bytes=1024

# Get logs since the last 5 minutes
runai workspace logs <workload-name> --since=300s

# Get logs since a specific timestamp
runai workspace logs <workload-name> --since-time=2023-05-30T10:00:00Z

# Wait up to 30 seconds for a workload to be ready for logs
runai workspace logs <workload-name> --wait-timeout=30s
```
...
```
GET https://run-ai-docs.nvidia.com/self-hosted/reference/cli/runai/runai_workspace_logs.md?ask=<question>
```
