...
view logs of a pytorch training workload
...
runai training pytorch logs <workload-name>

# Get logs for a specific pod in a workload
runai training pytorch logs <workload-name> --pod=<pod-name>

# Get logs for a specific container in a workload
runai training pytorch logs <workload-name> --container=<container-name>

# Get the last 100 lines of logs
runai training pytorch logs <workload-name> --tail=100

# Get logs with timestamps
runai training pytorch logs <workload-name> --timestamps

# Follow the logs
runai training pytorch logs <workload-name> --follow

# Get logs for the previous instance of the workload
runai training pytorch logs <workload-name> --previous

# Limit the logs to 1024 bytes
runai training pytorch logs <workload-name> --limit-bytes=1024

# Get logs since the last 5 minutes
runai training pytorch logs <workload-name> --since=300s

# Get logs since a specific timestamp
runai training pytorch logs <workload-name> --since-time=2023-05-30T10:00:00Z

# Wait up to 30 seconds for a workload to be ready for logs
runai training pytorch logs <workload-name> --wait-timeout=30s
```
...
```
GET https://run-ai-docs.nvidia.com/self-hosted/reference/cli/runai/runai_training_pytorch_logs.md?ask=<question>
```
