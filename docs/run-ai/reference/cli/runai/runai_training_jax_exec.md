# runai training jax exec

execute a command in a jax training workload

```
runai training jax exec [WORKLOAD_NAME] [flags]
```
...
runai training jax exec <workload-name> --tty --stdin -- /bin/bash 

# Execute ls command in the workload's main worker
runai training jax exec <workload-name> -- ls

# Execute a command in a specific worker of the workload
runai training jax exec <workload-name> --pod <pod-name> -- nvidia-smi
```
...
```
GET https://run-ai-docs.nvidia.com/self-hosted/reference/cli/runai/runai_training_jax_exec.md?ask=<question>
```
