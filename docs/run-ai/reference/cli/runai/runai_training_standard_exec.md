...
execute a command in a standard training workload
...
runai training standard exec <workload-name> --tty --stdin -- /bin/bash 

# Execute ls command in the workload's main worker
runai training standard exec <workload-name> -- ls

# Execute a command in a specific worker of the workload
runai training standard exec <workload-name> --pod <pod-name> -- nvidia-smi
```
...
```
GET https://run-ai-docs.nvidia.com/self-hosted/reference/cli/runai/runai_training_standard_exec.md?ask=<question>
```
