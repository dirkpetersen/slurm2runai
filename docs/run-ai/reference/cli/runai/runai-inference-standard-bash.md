# runai inference standard bash

open a bash shell in an inference workload

```
runai inference standard bash [WORKLOAD_NAME] [flags]
```
...
runai inference bash <workload-name>

# Open a bash shell in a specific pod of the workload
runai inference bash <workload-name> --pod <pod-name>
```
...
```
GET https://run-ai-docs.nvidia.com/self-hosted/reference/cli/runai/runai-inference-standard-bash.md?ask=<question>
```
