# runai inference nim exec

execute a command in a nim inference workload
...
```
runai inference nim exec [WORKLOAD_NAME] [flags]
```
...
runai inference nim exec <workload-name> --tty --stdin -- /bin/bash 

# Execute ls command in the workload
runai inference nim exec <workload-name> -- ls

# Execute a command in a specific pod of the workload
runai inference nim exec <workload-name> --pod <pod-name> -- nvidia-smi
```
...
```
GET https://run-ai-docs.nvidia.com/self-hosted/reference/cli/runai/runai-inference-nim-exec.md?ask=<question>
```
