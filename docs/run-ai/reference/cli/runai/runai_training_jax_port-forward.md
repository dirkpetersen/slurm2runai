# runai training jax port-forward

forward one or more local ports to a jax training workload

```
runai training jax port-forward [WORKLOAD_NAME] [flags]
```
...
runai training jax port-forward <workload-name> --port 8080:8090 --address localhost

# Forward connections from 0.0.0.0:8080 to a workload on port 8080:
runai training jax port-forward <workload-name> --port 8080 --address 0.0.0.0 [requires privileges]
...
runai training jax port-forward <workload-name> --port 8080:8090 --port 6443:443 --address localhost
```
...
```
GET https://run-ai-docs.nvidia.com/self-hosted/reference/cli/runai/runai_training_jax_port-forward.md?ask=<question>
```
